...
export class QwenStreamHandler {
  ...
  handleStream(stream: any, response?: AxiosResponse): PassThrough {
    ...
    // Decode stream chunks with StringDecoder so multi-byte UTF-8 characters split
    // across network chunk boundaries are not corrupted (missing/garbled characters)
        ...
        if (eventData && eventData !== '[DONE]') {
          try {
            ...
            if (result.data?.messages) {
              ...
              // The actual answer text lives in the multi_load/iframe message; text/plain messages
              // only carry think markers ([(deep_think)] etc.). Sharing a single this.content
              // accumulator across both message types makes thinking text pollute the answer
              // delta tracking, which causes duplicated / missing characters in the stream.
              const hasMultiLoadAnswer = result.data.messages.some((m: any) => m.mime_type === 'multi_load/iframe')
              for (const { msg } of eventMessages) {
                // Only treat text/plain as an answer source when this event has no multi_load/iframe
                if (msg.mime_type !== (hasMultiLoadAnswer ? 'multi_load/iframe' : 'text/plain')) continue
    ...
    decompressStream.on('data', (bufferChunk: Buffer) => {
      if (streamEnded) return
      // Use StringDecoder to avoid corrupting multi-byte UTF-8 characters split across chunk boundaries
            ...
            try {
              ...
              if (result.data?.messages) {
                ...
                for (const msg of result.data.messages) {
                  ...
                  // text/plain messages only carry think markers; use them as the answer source
                  // only when this event has no multi_load/iframe message (fallback for older API shapes)
                  if (msg.mime_type === 'text/plain' && msg.content && !hasMultiLoadAnswer) {
                    // Filter out deep_think and multimodal_chat_think markers
