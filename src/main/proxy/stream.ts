  ...
  createTransformStream(
    ...
    return new Transform({
      ...
      transform(chunk: Buffer, encoding, callback) {
        try {
          const events = parser.parse(decoder.write(chunk))
      ...
      flush(callback) {
        ...
        if (tail) {
          for (const event of parser.parse(tail)) {
  ...
  async streamToResponse(
    ...
    return new Promise((resolve, reject) => {
      ...
      stream.on('data', (chunk: Buffer) => {
        const events = this.parser.parse(decoder.write(chunk))

        for (const event of events) {
          if (event.data === '[DONE]') continue

          try {
            accumulate(JSON.parse(event.data))
          } catch {
            // Ignore parse errors
          }
        }
      ...
      stream.on('end', () => {
        ...
        const tail = decoder.end()
        if (tail) {
          for (const event of this.parser.parse(tail)) {
            if (event.data === '[DONE]') continue
            try {
              accumulate(JSON.parse(event.data))
            } catch {
